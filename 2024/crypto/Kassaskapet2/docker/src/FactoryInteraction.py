...
class Web3Handler:
    ...
    def createInstance(self, player):
        nonce = self.web3.eth.get_transaction_count(self.account.address)
        tx = self.factory.functions.createInstance(player).build_transaction({
            'chainId': 31337,
            'gas': 1000000,
    ...
    def validateInstance(self, contractAddress, playerAddress):
        ...
        if contractAddress + '\n' not in self.allowedaddrs:
            print("I have not created that contract, or it has already been claimed") #sanity check so you cant deploy your own Vault contracts and set yourself as owner
            exit(0)
        try:
...
#w3h = Web3Handler("0xE86F04FD7d6a16e1F4FB77A6e6fbc7661a673dAd", "0xdf3d2184679232ecf3c8dfc86719a1087a9d04b9e5af4a5953641b7572770f23")
...
if inp == '0':
    ...
    inst = w3h.createInstance(addr)
    print("\nHttpProvider: http://challs.crate.nu:50007")
    print("ChainId: 31337")
    print('Account address:', addr)
    print('Private Key:', pk.hex())
    print('Vault 2 contract instance:', inst)
...
if inp == '1':
    ...
    if w3h.validateInstance(ca, pa):
        print("cratectf{BlockHashTooPredictable!}")
    else:
